...
**Feature Components:**
- `src/components/openclaw-embedded-agent/index.tsx` - Main feature component

**Service Layer:**
- `src/services/openclaw-embedded-agentService.ts` - Business logic

**Technical Requirements:**
- Proxy requests to OpenClaw API with usage accounting and quota enforcement
- Store research jobs and generated outputs in DB with links to captured sources and metadata (timestamp, query, confidence)
- Web research pipeline: crawler/scraper + content extractor + source snapshot storage (archival)
- UI components to trigger agent, display progress, review sources and generated outputs, and insert into editor
- Implement safety guardrails: source quoting, hallucination detection heuristics, human-in-the-loop approval
- Rate-limiting, cost controls, and retry/backoff for API calls
...
- **Draft Content with OpenClaw Research:**
  - Trigger: User action
  - Result: Navigate to next step

**Integration Points:**
- Links to/from: page_010, page_011, page_005, page_008
...
- Content Studio (Editor) (OpenClaw Embedded Agent feature → Content Studio AI panel)
- Research Workspace (OpenClaw Embedded Agent feature → Research Workspace)
- OpenClaw API (OpenClaw feature → OpenClaw API for research/generation)
- Dashboard (implements)
- File Library (implements)

## Userflow

**Complete User Journey:**

1. **Access Dashboard** (step_page)
   - User lands on the main dashboard after authentication, viewing projects, deadlines, and research highlights.
   - Page type: Standard page

2. **Upload & Organize Files** (action)
   - User uploads documents/media to the File Library, applies tags, and reviews version history.
   - Action: User performs action
   - Expected result: Action completes successfully

3. **Draft Content with OpenClaw Research** (action)
   - User creates a new content item, triggers OpenClaw to research a topic, and links findings to their draft.
   - Action: User performs action
   - Expected result: Action completes successfully
...
```typescript
interface OpenClawEmbeddedAgent {
  id: string;
  user_id: string;
  title: string;
  description?: string;
...
**Functional:**
- [ ] OpenClaw Embedded Agent implemented per scope
- [ ] All elements present and functional
- [ ] User flows complete: Access Dashboard, Upload & Organize Files, Draft Content with OpenClaw Research
